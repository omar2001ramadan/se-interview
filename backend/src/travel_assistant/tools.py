...
def _result_domain(raw_result: dict[str, Any]) -> str:
    url = raw_result.get("href") or raw_result.get("url") or ""
    return urlparse(url).netloc.removeprefix("www.").lower()


def _rank_web_results(query: str, raw_results: list[dict[str, Any]]) -> list[dict[str, Any]]:
    filtered_results = [
        raw_result
        for raw_result in raw_results
        if not any(blocked_domain in _result_domain(raw_result) for blocked_domain in LOW_QUALITY_DOMAINS)
    ]
    raw_results = filtered_results or raw_results
    lowered_query = query.lower()
    preferred_domains: tuple[str, ...] = ()
    if "weather" in lowered_query or "forecast" in lowered_query:
        preferred_domains = WEATHER_DOMAINS
    elif "advisory" in lowered_query or "travel advisory" in lowered_query:
        preferred_domains = TRAVEL_ADVISORY_DOMAINS
    elif "flight" in lowered_query or "delay" in lowered_query:
        preferred_domains = AVIATION_DOMAINS

    if not preferred_domains:
        return raw_results

    def score(raw_result: dict[str, Any]) -> tuple[int, str]:
        domain = _result_domain(raw_result)
        preferred = any(preferred_domain in domain for preferred_domain in preferred_domains)
        return (0 if preferred else 1, domain)

    return sorted(raw_results, key=score)
...
def search_web(query: str, max_results: int = 5) -> str:
    ...
    lowered_query = query.lower()
    if "weather" in lowered_query or "forecast" in lowered_query:
        effective_query = f"{query} official weather forecast"
    raw_results = _perform_search(effective_query, max_results=max_results)
    raw_results = _rank_web_results(query, raw_results)
